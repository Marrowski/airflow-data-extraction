from confitest import api_key, channel_handle, mock_postgres_conn_vars, dagbag

def test_api_key(api_key):
    assert api_key == "NEW_MOCK_KEY"


def test_channel_handle(channel_handle):
    assert channel_handle == "VanillaImpact"
...
def test_dags_integrity(dagbag):
    ...
    expected_dag_ids = ["produce_json", "update_db", "data_check"]
    ...
    for dag_id, dag in dagbag.dags.items():
        ...
        ), f"DAG {dag_id} has {actual_count} of tasks, expected {expected_count}"
